...
const MCP_ENDPOINT = "https://example.test/book-of-ohdsi/mcp";
...
describe("createHttpHandler (Fetch API transport)", () => {
  ...
  it("responds to an initialize request with server info and capabilities", async () => {
    ...
    expect(payload.result.serverInfo.name).toBe("book-of-ohdsi");
    expect(payload.result.capabilities).toMatchObject({
      resources: {},
      tools: {},
    });
  });
